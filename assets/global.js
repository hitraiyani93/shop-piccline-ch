...
class VariantSelects extends HTMLElement {
  ...
  connectedCallback() {
    this.addEventListener('change', (event) => {
      ...
      this.handleUnavailableOption(event.target);
  ...
  handleUnavailableOption(target) {
        ...
        if (firstAvailable) {
          ...
          /*setTimeout(() => {
            ...
            if (addToCartButton) {
              ...
              if (buttonText) buttonText.textContent = 'In den Warenkorb';
            }
          }, 20);*/
          
          // Dispatch a new change event to update everything
          target.dispatchEvent(new Event('change', { bubbles: true }));
          return true; // Indicate that we handled an unavailable option
        }
      }
    }
    ...
    return false; // No unavailable option was handled
  }

  updateSelectionMetadata({ target }) {
    // If we're updating after handling an unavailable option, we need to work with the new target
    if (target.tagName === 'SELECT' && target.selectedOptions.length) {
      ...
      // Add selected attribute to the current selection
  ...
  initVariantAutoSwitch() {
    ...
    setTimeout(() => {
      this.querySelectorAll('select, input[type="radio"]').forEach(input => {
